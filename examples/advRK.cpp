...
public:
   FE_Evolution(SparseMatrix &_M, SparseMatrix &_K, LinearForm &_b, 
					 FunctionCoefficient &_inflow);
...
int main(int argc, char *argv[])
{
   ...
   FE_Evolution adv(m.SpMat(), k.SpMat(), b, inflow);
   ...
   bool done = false;
	...
	FunctionCoefficient exact(inflow);
	// Start timing
   ...
   for (int ti = 0; !done; )
   {
	...
	exact.SetTime(t);
	mfem::out << "|| u_h - u ||_{L^2} = " << u.ComputeL2Error(exact) << '\n';
...
FE_Evolution::FE_Evolution(SparseMatrix &_M, SparseMatrix &_K, LinearForm &_b, 
									FunctionCoefficient &_inflow): 
...
double u0_function(const Vector &X)
{
	double x = X(0), y = X(1), u;

	switch (problem)
   {
      case 0:
      case 1: u = cos(0.5*x + 0.5*y); break;
      case 2: u = cos(0.5*x + 0.5*y) - 0.5*cos(5*(0.5*x + 0.5*y)); break;
      case 3: u = asin(cos(0.5*x + 0.5*y)); break;
   }
   return 0.0;
	// return u;
}
...
double inflow_function(const Vector &X, const double t)
{
   ...
   switch (problem)
   {
      ...
      case 2: u = cos(0.5*x + 0.5*y - t) - 0.5*cos(5*(0.5*x + 0.5*y - t)); break;
